...
const Blogs: React.FC = () => {
  ...
  //unique categories
  const getUniqueCategories = (dataArray: Blog[]) => {
    const allCategories = dataArray
      .map(item => item.cat)
            ...
            paginatedBlogs.map((blog: Blog) => (
              ...
              >
                ...
                <div className="flex-1">
                  <h1 className="p-1 inline-block rounded-xl bg-gray-200 text-gray-500 text-sm sm:text-base mb-2">
                    {blog.cat}
                  </h1>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {blog.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-1 text-sm bg-gray-500 rounded-md"
                      >
                        #{tag}
                      </span>
                    ))}
        ...
        <div className="w-full lg:w-1/3 mt-10 lg:mt-0 space-y-10">
          ...
          <div className="text-center lg:text-left">
            ...
            <ul className="space-y-3">
              {
                getUniqueCategories(blogs).map((item, index) => (
                  <li key={index} className="px-4 py-2 bg-gray-500 rounded-lg">{item}</li>
                ))
              }
            </ul>
          </div>
